fn main() {
    ...
    if target == "wasm32-wasip2" {
        println!("cargo:rustc-link-lib=static=tree_sitter_gomod");
        println!("cargo:rustc-link-search=native={}/lib", manifest);
        println!("cargo:rerun-if-changed=lib/libtree_sitter_gomod.a");
        return;
    }
    ...
    cfg.file("src/parser.c").compile("tree_sitter_gomod");
